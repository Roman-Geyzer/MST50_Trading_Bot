...
@njit
def compute_profit(order_type, volume, entry_price, current_price, pip, contract_size):
	"""
	Compute profit in base currency before conversion.

	Parameters:
		order_type (int): Order type (buy or sell).
...
@njit
def check_sl_tp(order_type, sl, tp, high, low):
	sl_hit = False
	tp_hit = False
	exit_price = 0.0
...
class MT5Backtest:
	...
	def advance_time_step(self, current_time_index):
		...
		for symbol, tfs in self.symbols_data.items():
			for tf_name in tfs.keys():
				times = self.symbols_data[symbol][tf_name]['time']
				current_index = np.searchsorted(times, self.current_time, side='right') - 1
	...
	def simulate_target_hit_close(self):
		tickets_to_check = list(self.open_positions.keys())
		for ticket in tickets_to_check:
			...
			# Use the Numba-compiled function to check SL/TP
			sl_hit, tp_hit, exit_price = check_sl_tp(order_type, sl, tp, high, low)
	...
	def calculate_profit(self, position):
		...
		# Compute profit in base currency using Numba-compiled function
		profit = compute_profit(order_type, volume, entry_price, current_price, pip, contract_size)
		...
		if quote_currency != 'USD':
			...
			if quote_currency in usd_base_currencies:
				...
				conversion_price = self.get_current_price(conversion_symbol, ORDER_TYPE_BUY)
				conversion_rate = conversion_price
				...
				if conversion_price and conversion_price > 0:
					conversion_rate = 1 / conversion_price
				else:
					conversion_rate = None
			else:
